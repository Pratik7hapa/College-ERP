...
def student_view_result(request):
    ...
    test_scores = []
    exam_scores = []
    completed_subject_ids = []  # track which subjects have results
    for result in results:
        ...
        completed_subject_ids.append(result.subject_id)

    overall_gpa = total_gpa / len(results_with_data) if results_with_data else 0.0

    total_subjects_qs = Subject.objects.filter(course=student.course)
    total_subjects = total_subjects_qs.count()
    completed_subjects = len(results_with_data)
    remaining_subjects = total_subjects - completed_subjects

    # Determine remaining subjects (those without results)
    remaining_subjects_qs = total_subjects_qs.exclude(id__in=completed_subject_ids)
    ...
    target_gpa_table = []  # list of dicts for each remaining subject
    if request.method == 'POST' and 'target_gpa' in request.POST:
        try:
            ...
            if remaining_subjects > 0:
                ...
                for subject in remaining_subjects_qs:
                    target_gpa_table.append({
                        'subject': subject.name,
                        'required_internal': required_internal_per_subject,
                        'required_final': required_final_per_subject,
                        'required_total': required_total_marks_per_subject,
                    })
            else:
